...
public class AbstractReadWriteRedisAccessStrategy<T extends RedisTransactionalDataRegion>  extends AbstractRedisAccessStrategy<T> {
    ...
    public final SoftLock lockItem(Object key, Object version) throws CacheException {
        ...
        try {
            ...
            final Lock lock;
            if (item == null) {
            	lock = new Lock(timeout, uuid, nextLockId(), version); 
            } else {
            	lock = item.lock(timeout, uuid, nextLockId());
            }
            region.put(key, lock);
            return lock;
        } finally {
    ...
    protected void decrementLock(Object key, Lock lock) {
        logger.trace("Decrementing lock.... key=[{}], lock=[{}]", key, lock);
    ...
    protected void handleLockExpiry(Object key, Lockable lock) {
        ...
        final Lock newLock = new Lock(
        	timeStamp, uuid, nextLockId.getAndIncrement(), null);
